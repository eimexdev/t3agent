...
Chromium's MediaRecorder omits seekable duration metadata from audio-only WebM recordings. T3 Agent repairs that metadata locally before creating the draft and lazily repairs older raw recordings before playback. The live meter is a fixed-density rolling window; the draft and sent bubble use peaks sampled across the full recording.

The installed Hermes STT path has a real 25 MiB per-file ceiling and no duration ceiling. The T3 adapter advertises that decoded-file limit and gives its inline JSON ingress enough room for base64 expansion rather than introducing an unrelated product timer.

The sent message appears immediately as a playable voice bubble. The controlled Hermes adapter emits attachment-correlated transcription lifecycle events around Hermes's existing pre-agent STT work. T3 renders those actual states and the Hermes-produced text beneath the matching bubble: short transcripts expanded by default and long transcripts collapsed. This is bridge metadata around Hermes's transcription result, not a T3 transcription pipeline. Transcripts are joined by stable voice-attachment identity, never by guessing that the next text message belongs to the recording.
...
On web, a recording survives SPA thread navigation and background tabs. Another thread shows a compact recording indicator with Return and Stop controls; Return uses client-side navigation so capture is not interrupted. Periodic IndexedDB checkpoints recover an interrupted recording as a local draft. Background recording is also required on mobile, including the native iOS background-audio mode and Android foreground microphone service/recording notification. Web voice work may ship before the mobile product pass.
